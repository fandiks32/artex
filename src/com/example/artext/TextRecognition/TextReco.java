...
import com.example.artext.SampleApplication.SampleApplicationControl;
import com.example.artext.SampleApplication.SampleApplicationException;
...
import com.example.artext.TextRecognition.TextRecoRenderer.WordDesc;
...
import com.example.artext.ui.SampleAppMenu.SampleAppMenuGroup;
import com.example.artext.ui.SampleAppMenu.SampleAppMenuInterface;
    ...
    SampleAppMenuInterface
{
    ...
    public final static int WORDLIST_MARGIN = 10;
    ...
    public ArrayList<translatedWords> targetedTranslatedWord = new ArrayList<translatedWords>();
    public String translatedNow;
    private ArrayList<String> TargetedWord = new ArrayList<String>() ;
    private int index = 0;
    private ArrayList<String> tmpTargetedWord = new ArrayList<String>() ;
    private int tmpCountWord1=0;
    private int tmpCountWord2=0;
    ...
    public StringBuilder stringBuilderDetector = null;
    public StringBuilder stringBuilderDictionary = null;
    ...
    private void initKamus(){
    	 //untuk inialisasi data wordlist
    	 tmpTargetedWord.add(0, new String("empty"));
         tmpTargetedWord.add(1, new String("empty"));
         AssetManager assetManager = getApplicationContext().getResources().getAssets();
		 InputStream inputStream = null;
		 try {
			inputStream = assetManager.open("TextReco/addWord.txt");
			if(stringBuilderDetector==null){
				BufferedReader r = new BufferedReader(new InputStreamReader(inputStream));
				stringBuilderDetector = new StringBuilder();
				String line;
				while ((line = r.readLine()) != null) {
					stringBuilderDetector.append(line+"\t\n");
				}
			}
        } catch (IOException e) {
		...
		try {
			inputStream = assetManager.open("TextReco/dataDummy.txt");
			if(stringBuilderDictionary==null){
				BufferedReader r = new BufferedReader(new InputStreamReader(inputStream));
				stringBuilderDictionary = new StringBuilder();
				String line;
	       	while ((line = r.readLine()) != null) {
	       		stringBuilderDictionary.append(line+"\n");
	       	}
        }
        } catch (IOException e) {
			// TODO Auto-generated catch block
            ...
            public void run()
            {
                ...
                if (words.size() > 0)
                {
                    ...
                    for (WordDesc word : words)
                    {
                        ...
                        try {
                        	//ketika data terjemahan terseedia
                        	cycle++;
                        	if(cycle==3){
                        		tmpCountWord1=0;
                        		return;
                        	}
                        	if(tmpTargetedWord.get(0).equals(word.text)||tmpTargetedWord.get(1).equals(word.text)){
                        		tmpCountWord1++;
                        		if(tmpCountWord1==2){
                        			
                        			//tampil translate
                        		//	Toast.makeText(getApplicationContext(), 
                        				//	"sukses 0/1"+TargetedWord.get(0)+"/"+TargetedWord.get(1)+"size"+tmpTargetedWord.size(),
                        			//		Toast.LENGTH_SHORT).show();
                        			//translate
                        			if(targetedTranslatedWord.size()!=0){
                        				
                        				for(int c=0;c<targetedTranslatedWord.size();c++){
                        					if((targetedTranslatedWord.get(c).text1.equals(tmpTargetedWord.get(0))||
                        							targetedTranslatedWord.get(c).text2.equals(tmpTargetedWord.get(0)) && 
                        							(targetedTranslatedWord.get(c).text1.equals(tmpTargetedWord.get(1))||
                        							 targetedTranslatedWord.get(c).text2.equals(tmpTargetedWord.get(1))))
                        							){
                        						//tampil
                        						tv = new TextView(TextReco.this);
                                                tv.setText(targetedTranslatedWord.get(c).translated);
                                                translatedNow = targetedTranslatedWord.get(c).translated;
                        					...
                        					//tambah dan translate
                        					if((c+1)==targetedTranslatedWord.size()){
                        						String resulted = FileOperations.searchWord(
                        								tmpTargetedWord.get(0).toLowerCase(), 
                        								tmpTargetedWord.get(1).toLowerCase(), 
                        								getApplicationContext(), 
                        								stringBuilderDictionary);
                                				if(resulted!=null){
                                					//tampil
                                					translatedWords tw = new translatedWords(resulted,tmpTargetedWord.get(0),tmpTargetedWord.get(1));
                                					targetedTranslatedWord.add(tw);
                                					tv = new TextView(TextReco.this);
                                                    tv.setText(resulted);
                                                    translatedNow = resulted;
                                                    RelativeLayout.LayoutParams txtParams = new RelativeLayout.LayoutParams(
                                                        LayoutParams.MATCH_PARENT,
                                                        LayoutParams.WRAP_CONTENT);
                        				...
                        				String resulted = FileOperations.searchWord(
                        									...
                        									tmpTargetedWord.get(1).toLowerCase(), 
                        										getApplicationContext(), 
                        										stringBuilderDictionary);
                        				//Toast.makeText(getApplicationContext(), "hasil:"+resulted, Toast.LENGTH_LONG).show();
                        	...
                        	//untuk menambahkan di targetWOrd atau tmp targetword
                        	if(TargetedWord.size()!=0){
                        	//	Toast.makeText(getApplicationContext(), 
                    			//		"app 0/1"+tmpTargetedWord.get(0)+"/"+tmpTargetedWord.get(1)+"size"+TargetedWord.size(),
                    				//	Toast.LENGTH_SHORT).show();
                        		for(int c=0;c<TargetedWord.size();c++){
                            		if(TargetedWord.get(c).equals(word.text)){
                            			tmpTargetedWord.set(tmpCountWord2++, new String(TargetedWord.get(c)));
                            			if(tmpCountWord2==2){
                            				tmpCountWord2=0;
                            			}
                            			break;
                            		}else{
                            			if(FileOperations.checkTargetedWord(word.text, stringBuilderDetector)){
                            				if(!TargetedWord.contains(word.text)){
                                    			TargetedWord.add(index++,new String(word.text));
                            				}
                            			}
                            		}
                            	}
    ...
    private void showLoupe(boolean isActive)
    {
        ...
        //digunakan untuk mengatur layar text recognition
        //int loupeHeight = mIsTablet ? (height * 10) / 100 : (height * 15) / 100;
        //int loupeHeight = mIsTablet ? (height * 10) / 100 : (height * 90) / 100;
        int loupeHeight = (height*70)/100;
        // lupue width takes the width of the screen minus 2 margins
